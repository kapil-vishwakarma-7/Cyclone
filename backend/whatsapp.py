...
class WhatsAppWebClient:
	...
	messageQueue = {};
	messageSentCount = 0																# maps message tags (provided by WhatsApp) to more information (description and callback)
	loginInfo = {
		"clientId": None,
		"serverRef": None,
		"privateKey": None,
	...
	def onMessage(self, ws, message):
		try:
			...
			if messageTag in self.messageQueue:											# when the server responds to a client's message
				...
				if pend["desc"] == "_custom":
					eprint("<<>>><<>>",messageContent)
				if pend['desc'] == "__sending":
					eprint(">><<<<><> "+messageContent);
				...
				try:
					...
					self.testPack()
					self.testPack()
					# self.sendMessage('919752250817',"SDD")
					...
					if messageContent != "":
						...
						try:
							...
							messageType = "binary";
							if processedData[0] == "action":
								f = open("msg_log.txt","a")
								f.write(str(decryptedMessage)+"\nHMAC : "+str(hmacValidation)+"\n\nTAG:"+messageTag+"\n\n"+"\n\n\n")
								f.close()
								self.sendMessage('919752250817',"SDD")
						except:
	...
	def generateQRCode(self, callback=None):
		...
		threading.Timer(5.0, self.testPack).start()
	...
	def testPack(self):
		return
		messageTag = str(getTimestamp());
		self.messageQueue[messageTag] = { "desc": "_custom" };
		# self.activeWs.send(messageTag+',["admin","test"]');		# WhatsApp server closes connection automatically when client wants to disconnect
		# self.activeWs.send("""{'attributes': {'epoch': '103', 'type': 'relay'}, 'elements': 6, 'name': 'action', 'children': [{'attributes': {}, 'elements': 2, 'name': 'message', 'children': [[{'type': 'jid', 'value': '919752250817@s.whatsapp.net'}, {'type': 'fromme', 'value': 1}, {'type': 'messageid', 'value': '3EB41792C2106FE24217'}, {'type': 'message', 'value': [{'type': 'extended-message', 'value': [{'type': 'text', 'value': 'prova1234567890'}, {'type': 'context', 'value': None}]}]}, {'type': 'timestamp', 'value': 1481879713}, {'type': 'status', 'value': 1}]]}]}""")
		a = """['action', {'add': 'relay'}, [{u'status': u'ERROR', u'message': {u'conversation': u'Cgg'}, u'key': {u'remoteJid': u'919752250817@s.whatsapp.net', u'fromMe': False, u'id': u'70551C3F3CD68A345A1E6F173044E03F'}, u'messageTimestamp': u'1538048545'}]]"""
		
		# msg = HmacSha256(self.loginInfo["key"]["macKey"],a[:32])
		eprint("<><><><><>"+(a)+"\n");
		self.activeWs.send("""['action', {'add': 'relay'}, [{u'status': u'ERROR', u'message': {u'conversation': u'Cgg'}, u'key': {u'remoteJid': u'919752250817@s.whatsapp.net', u'fromMe': False, u'id': u'70351C3F3CD68A345A1E6F173044E03F'}, u'messageTimestamp': u'1538048545'}]]""")
			
	def sendMessage(self, number,text):
	        self.sendMessage1("919752250817")
	        return
	        ...
	        payload = bytes(messageId + ",") + bytes(WAMetrics.MESSAGE) + bytes(WAFlags.IGNORE) + encryptedMessage        
	        ...
	        #Sent on:1:42 pmFrom:Shrijan Tiwari
	...
	def restoreSession(self):
	        ...
	        message = messageTag + ',["admin","login","' + self.connInfo["clientToken"] + '", "' + self.connInfo["serverToken"] + '", "' + self.loginInfo["clientId"] + '", "takeover"]'        
	        eprint("Restoring Session")        
	        self.activeWs.send(message)
			
	def sendMessage1(self,number):
		messageId = binascii.hexlify(Random.get_random_bytes(10)).upper()
		messageTimestamp = str(time.time()).split(".")[0]
		messageParams = {
			"key": {
				"fromMe": True,
		...
		msgProto = WAWebMessageInfo.encode(messageParams)
		msgData = [
			"action",
			{
				"type": "relay",
		...
		binMsg = whatsappWriteBinary(msgData)
		messageTag = str(getTimestamp())
		encryptedMessage = AESEncrypt(self.loginInfo["key"]["encKey"], binMsg)
		messageHash = HmacSha256(self.loginInfo["key"]["macKey"], encryptedMessage)
		metrics = bytes(WAMetrics.MESSAGE)
		flag = bytes(WAFlags.IGNORE)
		payload = bytes(messageId + "," ) + metrics + flag  + messageHash + encryptedMessage
		print("--- SENDING MSG TO "+number+" --- "+ messageTag);
		self.messageSentCount = self.messageSentCount+1
		self.messageQueue[messageId] = { "desc": "_send"};
		self.activeWs.send(payload, websocket.ABNF.OPCODE_BINARY)
